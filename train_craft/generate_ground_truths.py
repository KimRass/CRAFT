...
def generate_score_map(img, poly, gaussian_map, margin=0.3):
    ...
    M = cv2.getPerspectiveTransform(src=rect, dst=poly.astype("float32"))
...
def generate_affinity_score_map(img, annots, gaussian_map, margin=0.3):
    ...
    # pts1 = np.array([[0, 0], [w, 0], [w, h], [0, h]], dtype="float32")
