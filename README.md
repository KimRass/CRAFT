...
### Loss
$$L = \sum_{p}{S_{c}(p) \cdot \big(\lVert S_{r}(p) - S_{r}^{*}(p) \rVert_{2}^{2} + \lVert S_{a}(p) - S_{a}^{*}(p)\rVert_{2}^{2}\big)}$$
...
- Comment: 그런데 사실상 Region score map과 Affinity score map은 2차원이므로 $p$에서 스칼라를 가집니다. 따라서 $L$은 그냥 다음과 같습니다.
